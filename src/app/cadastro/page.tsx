...
export default function CadastroPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    const supabase = createClient(); const { data, error: signUpError } = await supabase.auth.signUp({ email, password, options: { data: { display_name: name } } });
    ...
    const response = await fetch("/api/cadastro", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: data.user.id, name, email, cpf, birthDate, phone, cep, street, number, complement, neighborhood, city, uf, ibgeCode }) });
    const result = await response.json().catch(() => ({}));
    if (!response.ok) { setError(result.error || "Não foi possível concluir o cadastro."); setBusy(false); return; }
    setMsg("Cadastro realizado. Verifique seu e-mail para confirmar a conta."); setBusy(false);
  }
