...
export async function GET(_request: Request, { params }: Props) {
  ...
  if (!user) return NextResponse.json({ error: "É necessário estar autenticado para visualizar este plano." }, { status: 401 });
  ...
  if (!plan || plan.status !== "active") return NextResponse.json({ error: "Este plano não está disponível para assinatura." }, { status: 404 });
  ...
  if (!creator || creator.status !== "active") return NextResponse.json({ error: "O criador não está disponível." }, { status: 404 });
  if (creator.user_id === user.id) return NextResponse.json({ error: "O criador não pode assinar o próprio plano." }, { status: 409 });
