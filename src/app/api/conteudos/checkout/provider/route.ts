...
export async function POST(request: Request) {
  ...
  if (order.metadata?.product_type !== "digital_content") return NextResponse.json({ error: "Pedido não é de conteúdo digital." }, { status: 409 });
  ...
  try {
    const preference = await createMercadoPagoPreference({
      ...
      title: typeof order.metadata?.title === "string" ? order.metadata.title : "Conteúdo digital Pecatho",
      amount: Number(order.total),
      buyerEmail: user.email ?? "",
    });
    await admin.from("payments").update({
      provider: "mercadopago",
      raw_reference: { ...(payment.raw_reference && typeof payment.raw_reference === "object" ? payment.raw_reference : {}), channel: "pecatho_digital_content", provider: "mercadopago", preference_id: preference.id, order_id: order.id },
      updated_at: new Date().toISOString(),
    }).eq("id", payment.id);
    return NextResponse.json({ provider: "mercadopago", preference_id: preference.id, checkout_url: preference.init_point ?? preference.sandbox_init_point ?? null, sandbox: !preference.init_point && Boolean(preference.sandbox_init_point) });
  } catch (error) {
    return NextResponse.json({ error: "Não foi possível criar o checkout no provedor.", detail: error instanceof Error ? error.message : "Erro desconhecido." }, { status: 502 });
  }
}
