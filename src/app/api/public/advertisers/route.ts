  ...
  try {
    ...
    if (mode === "meta") {
      ...
      return NextResponse.json(
        { categories: categories.data ?? [], states: states.data ?? [] },
    ...
    let query = supabase
      ...
      .select("id,slug,title,display_name,summary,city_id,state_id,category_id,verification_status,primary_media_id,created_at")
      .eq("status", "published")
    ...
    if (q) {
      ...
      query = query.or(`title.ilike.%${clean}%,display_name.ilike.%${clean}%,summary.ilike.%${clean}%`);
    }
