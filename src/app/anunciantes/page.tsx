...
type Advertiser = { id: string; slug: string | null; title: string | null; display_name: string | null; summary: string | null; city_id: number | null; state_id: number | null; category_id: number | null; verification_status: string | null; pricing: Record<string, unknown> | null; city_name: string | null; state_uf: string | null; total_count: number | null };
...
export default function AnunciantesPage() {
  const [advertisers, setAdvertisers] = useState<(Advertiser & { imageUrl: string | null })[]>([]); const [totalCount, setTotalCount] = useState(0);
  ...
  const [categoryId, setCategoryId] = useState(""); const [stateId, setStateId] = useState(""); const [cityId, setCityId] = useState(""); const [query, setQuery] = useState(""); const [searchQuery, setSearchQuery] = useState("");
  ...
  useEffect(() => {
    ...
    setCategoryId(nextCategory); setStateId(nextState); setCityId(nextCity); setQuery(nextQuery); setSearchQuery(nextQuery);
  ...
  return <main className="discoveryShell">
    ...
    <section className="discoveryHero"><div className="eyebrow">PECATHO · ANUNCIANTES</div><h1>Descubra perfis que combinam <em>com você.</em></h1><p className="heroCopy discoveryHeroCopy">Explore anunciantes, serviços, localização, características, disponibilidade e faixas de preço em uma experiência visual premium.</p><div className="advertiserHeroIntro"><div><span className="advertiserHeroKicker">EXPERIÊNCIAS ADULTAS · DESCOBERTA</span><strong>Encontre. Conheça. Escolha.</strong><p>Perfis publicados com informações organizadas para você descobrir novas possibilidades com mais clareza.</p></div><div className="advertiserHeroStats"><span><b>{totalCount}</b><small>perfis nesta busca</small></span><span><b>{categories.length}</b><small>categorias</small></span><span><b>{states.length}</b><small>estados</small></span></div></div>
