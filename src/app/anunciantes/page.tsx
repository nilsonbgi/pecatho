...
export default function AnunciantesPage() {
  ...
  useEffect(() => {
    ...
    setPriceMin(params.get("precoMin") || ""); setPriceMax(params.get("precoMax") || ""); setDurationMinutes(params.get("duracao") || "");
  ...
  function clearAll() { setCategoryId(""); setStateId(""); setCityId(""); setQuery(""); setAttributeFilters({}); setServiceFilters([]); setAgeMin(""); setAgeMax(""); setPriceMin(""); setPriceMax(""); setDurationMinutes(""); }
  ...
  return <main className="discoveryShell">
    ...
    <section className="discoveryResults"><div className="resultsHeader"><div><div className="eyebrow">PERFIS PUBLICADOS</div><h2>{loading ? "Encontrando perfis..." : `${advertisers.length} ${advertisers.length === 1 ? "perfil encontrado" : "perfis encontrados"}`}</h2>{(stateId || cityId) && <div className="resultsMeta">{cityId ? `${cityName.get(Number(cityId)) || ""}, ` : ""}{stateId ? stateName.get(Number(stateId)) || "" : ""}</div>}</div>{(categoryId || stateId || cityId || query || activeFilterCount) && <button type="button" className="secondaryButton" onClick={clearAll}>Limpar filtros</button>}</div>
      ...
      {!error && advertisers.length > 0 && <div className="advertiserGrid">{advertisers.map((item) => <article className="advertiserCard" key={item.id}><div className="advertiserVisual">{item.imageUrl ? <img src={item.imageUrl} alt={item.display_name || item.title || "Perfil Pecatho"} /> : <div className="visualPlaceholder"><span>✦</span><div>Pecatho</div><small>Mídia de apresentação</small></div>}{item.verification_status === "verified" && <span className="verifiedMark">✓ VERIFICADO</span>}</div><div className="advertiserBody"><div className="advertiserCategory">{item.category_id != null ? categoryName.get(item.category_id) || "ANUNCIANTE" : "ANUNCIANTE"}</div><h3>{item.title || item.display_name || "Perfil Pecatho"}</h3><p className="advertiserName">{item.display_name || "Anunciante"}</p><p className="advertiserLocation">⌖ {item.city_id != null ? cityName.get(item.city_id) || "Cidade não informada" : "Cidade não informada"}{item.state_id != null && stateName.get(item.state_id) ? ` · ${stateName.get(item.state_id)}` : ""}</p><p className="advertiserSummary">{item.summary || "Perfil publicado no Pecatho."}</p>{(() => { const periods = Array.isArray(item.pricing?.periods) ? (item.pricing.periods as PricingPeriod[]) : []; const valid = periods.filter((row) => Number.isFinite(Number(row.price)) && Number(row.price) > 0 && Number.isFinite(Number(row.minutes))); const oneHour = valid.find((row) => Number(row.minutes) === 60); const display = oneHour || valid[0]; return display ? <div className="advertiserCardPrice"><span>{display.starting_from === true ? "A partir de " : ""}{display.period || (Number(display.minutes) === 60 ? "1 Hora" : `${display.minutes} min`)}</span><strong>{new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(Number(display.price))}</strong></div> : null; })()}{item.slug && <Link className="primaryButton" href={`/anunciantes/${item.slug}`}>Ver perfil</Link>}</div></article>)}</div>}
    </section>
  </main>;
}
