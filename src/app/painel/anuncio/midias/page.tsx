...
type Media = { id: string; kind: "image" | "video"; storage_bucket: string; storage_path: string; original_filename: string | null; access_type: "public" | "paid"; price: number; currency: string; is_public: boolean; is_primary: boolean; moderation_status: string; sort_order: number };
...
export default function AdvertiserMediaManager() {
  ...
  async function load() {
    ...
    const { data, error: mediaError } = await supabase.from("profile_media").select("id,kind,storage_bucket,storage_path,original_filename,access_type,price,currency,is_public,is_primary,moderation_status,sort_order").eq("profile_id", profile.id).order("sort_order");
