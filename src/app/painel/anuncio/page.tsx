...
export default function AnuncioPage() {
  ...
  async function uploadMedia(e: React.ChangeEvent<HTMLInputElement>) {\n    const files = Array.from(e.target.files || []); if (!files.length) return; setMediaBusy(true); setError(""); setMessage("");\n    try { const supabase=createClient(); const {data:{user}}=await supabase.auth.getUser(); if(!user) throw new Error("Sessão expirada.");\n      const {data: profile}=await supabase.from("advertiser_profiles").select("id").eq("user_id",user.id).maybeSingle(); if(!profile) throw new Error("Salve primeiro o rascunho do anúncio.");\n      const current=media.length; let order=current;\n      for(const file of files){ if(!file.type.startsWith("image/") && !file.type.startsWith("video/")) throw new Error("Envie apenas imagens ou vídeos."); if(file.size>50*1024*1024) throw new Error("Cada arquivo deve ter no máximo 50 MB."); const kind=file.type.startsWith("video/")?"video":"image"; const ext=(file.name.split(".").pop()||"bin").toLowerCase(); const path=user.id+"/"+profile.id+"/"+crypto.randomUUID()+"."+ext; const up=await supabase.storage.from("pecatho-media").upload(path,file,{contentType:file.type,upsert:false}); if(up.error) throw up.error; const ins=await supabase.from("profile_media").insert({profile_id:profile.id,kind,storage_bucket:"pecatho-media",storage_path:path,original_filename:file.name,mime_type:file.type,size_bytes:file.size,sort_order:order,is_primary:order===0,is_public:true,moderation_status:"pending"}).select("*").single(); if(ins.error) throw ins.error; order++; }\n      const {data}=await supabase.from("profile_media").select("*").eq("profile_id",profile.id).order("sort_order"); setMedia(data||[]); setMessage("Mídia enviada e encaminhada para moderação.");\n    } catch(err){ setError(err instanceof Error?err.message:"Não foi possível enviar a mídia."); } finally { setMediaBusy(false); e.target.value=""; }\n  }\n\n  async function submit(e: FormEvent) {
