...
export default function AnuncioPage() {
  ...
  const [birthDate, setBirthDate] = useState(""); const [age, setAge] = useState(""); const [height, setHeight] = useState(""); const [weight, setWeight] = useState("");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const supabase = createClient();
        const [{ data: stateData, error: stateError }, { data: categoryData, error: categoryError }, { data: profile, error: profileError }, { data: address, error: addressError }] = await Promise.all([
        ...
        if (!alive) return;
        if (stateError || categoryError || profileError || addressError) setError("Não foi possível carregar todos os dados do anúncio.");
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      const missing = attributes.filter((a) => { const value = attributeValues[a.id]; return a.required && (value === undefined || value === null || value === "" || (Array.isArray(value) && value.length === 0)); }); if (missing.length) throw new Error(`Preencha os campos obrigatórios: ${missing.map((a) => a.name).join(", ")}.`);
      ...
      const parsedHeight = height.trim() ? Number(height.replace(",", ".")) : null; const parsedWeight = weight.trim() ? Number(weight.replace(",", ".")) : null; const parsedAge = age.trim() ? Number(age) : null;
      if (parsedAge != null && (!Number.isInteger(parsedAge) || parsedAge < 18 || parsedAge > 99)) throw new Error("Informe uma idade válida entre 18 e 99 anos."); if (parsedHeight != null && (!Number.isFinite(parsedHeight) || parsedHeight < 100 || parsedHeight > 250)) throw new Error("Informe uma altura válida entre 100 e 250 cm."); if (parsedWeight != null && (!Number.isFinite(parsedWeight) || parsedWeight < 30 || parsedWeight > 300)) throw new Error("Informe um peso válido entre 30 e 300 kg.");
      let effectiveBirthDate = birthDate || null; if (!effectiveBirthDate && parsedAge != null) { const d = new Date(); d.setUTCFullYear(d.getUTCFullYear() - parsedAge); effectiveBirthDate = d.toISOString().slice(0, 10); }
