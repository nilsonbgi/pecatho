...
export default function ReviewPage() {
  ...
  async function openAdvertiser(item: Advertiser) {
    ...
    const previews = await Promise.all(rows.map(async (row) => { const signed = await supabase.storage.from(row.storage_bucket).createSignedUrl(row.storage_path, 600); return { ...row, preview_url: signed.data?.signedUrl || undefined }; }));
    setMedia(previews); setVerification((verificationResult.data || null) as Verification | null); setAccount((accountResult.data || null) as Record<string, unknown> | null); setAddress((addressResult.data || null) as Record<string, unknown> | null);
  ...
  async function moderateMedia(mediaId: string, action: "approve" | "reject" | "pending") {
    if (!selected) return;
    setBusy(true); setError(""); setMessage("");
    try {
      const { data, error: rpcError } = await supabase.rpc("admin_moderate_profile_media", { p_media_id: mediaId, p_action: action });
      if (rpcError) throw rpcError;
      if (!data) throw new Error("A moderação da mídia não retornou o registro atualizado.");
      setMedia((current) => current.map((item) => item.id === mediaId ? { ...item, moderation_status: data.moderation_status } : item));
      setMessage("Status da mídia atualizado.");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Não foi possível moderar a mídia.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function moderate(action: "verify" | "reject" | "publish" | "pause") {
    ...
    setBusy(true); setError(""); setMessage("");
    try {
      const { data, error: rpcError } = await supabase.rpc("admin_moderate_advertiser", { p_profile_id: selected.id, p_action: action });
      if (rpcError) throw rpcError;
      if (!data) throw new Error("A moderação não retornou o anúncio atualizado.");
      const updated = data as Advertiser;
      setSelected(updated);
      setAdvertisers((current) => current.map((item) => item.id === updated.id ? updated : item));
      setMessage("Ação administrativa concluída com segurança.");
      await openAdvertiser(updated);
