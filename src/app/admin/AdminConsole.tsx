...
export default function AdminConsole({ role }: { role: string }) {
  ...
  const [planForm, setPlanForm] = useState({ id: "", name: "", description: "", duration_days: "30", price: "0,00", free: false, active: true });
  ...
  return <main className="shell">
    ...
    <section className="hero" style={{ maxWidth: 1400 }}>
      ...
      {loading ? <div className="authCard"><p>Carregando dados administrativos...</p></div> : <>
        ...
        {tab === "planos" && <section className="authCard"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}><div><h2>Planos de publicação</h2><p className="fieldNote">{planForm.id ? "Editando plano existente. As alterações serão gravadas no mesmo plano." : "Cadastre e gerencie os planos disponíveis para os anunciantes."}</p></div>{planForm.id && <button className="secondaryButton" type="button" onClick={() => setPlanForm({ id: "", name: "", description: "", duration_days: "30", price: "0,00", free: false, active: true })}>Cancelar edição</button>}</div><form onSubmit={savePlan}><div className="formRow"><label>Nome<input required value={planForm.name} onChange={e => setPlanForm({ ...planForm, name: e.target.value })} /></label><label>Duração (dias)<input type="number" min="1" value={planForm.duration_days} onChange={e => setPlanForm({ ...planForm, duration_days: e.target.value })} /></label><label>Valor (R$)<input inputMode="decimal" placeholder="0,00" value={planForm.price} disabled={planForm.free} onChange={e => setPlanForm({ ...planForm, price: e.target.value.replace(/[^0-9,.]/g, "") })} /></label></div><label>Descrição<textarea rows={3} value={planForm.description} onChange={e => setPlanForm({ ...planForm, description: e.target.value })} /></label><label><input type="checkbox" checked={planForm.free} onChange={e => setPlanForm({ ...planForm, free: e.target.checked, price: e.target.checked ? "0,00" : planForm.price })} /> Plano gratuito</label><label><input type="checkbox" checked={planForm.active} onChange={e => setPlanForm({ ...planForm, active: e.target.checked })} /> Ativo</label><button className="primaryButton">{planForm.id ? "Atualizar plano" : "Criar plano"}</button></form><hr /><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr><th>Plano</th><th>Duração</th><th>Valor</th><th>Status</th><th>Ações</th></tr></thead><tbody>{plans.map(p => <tr key={p.id}><td>{p.name}</td><td>{p.duration_days ? `${p.duration_days} dias` : "—"}</td><td>{p.free ? "Grátis" : `R$ ${Number(p.price).toFixed(2).replace(".", ",")}`}</td><td>{p.active ? "Ativo" : "Inativo"}</td><td><button className="secondaryButton" type="button" onClick={() => setPlanForm({ id: p.id, name: p.name, description: p.description || "", duration_days: String(p.duration_days || 30), price: Number(p.price).toFixed(2).replace(".", ","), free: p.free, active: p.active })}>Editar</button></td></tr>)}</tbody></table></section>}
