...
type Plan = { id: string; name: string; description: string | null; price: number; currency: string; interval: string; status: string };

type Card = {
  id: string; title: string; body: string | null; price: number; currency: string; access_type: "free" | "paid" | "subscriber";
  published_at: string | null; created_at: string; previewUrl: string | null; previewType: string | null;
...
function intervalLabel(value: string) {
  if (value === "monthly") return "por mês";
  if (value === "quarterly") return "por trimestre";
  if (value === "yearly") return "por ano";
  return "por período";
}
...
export default async function FansCreatorPublicPage({ params }: Props) {
  ...
  const [{ data: posts }, { data: plans }] = await Promise.all([
    ...
    admin.from("fans_plans").select("id,name,description,price,currency,interval,status").eq("creator_id", creator.id).eq("status", "active").order("price", { ascending: true }),
  ]);
  ...
  const activePlans = ((plans ?? []) as Plan[]).filter(plan => plan.status === "active");
