...
export default async function FansPage() {
  ...
  const { data: creator } = await supabase.from("fans_creators").select("id,slug,display_name,bio,status,avatar_url").eq("user_id", user.id).maybeSingle();
  let plansCount = 0;
  let postsCount = 0;
