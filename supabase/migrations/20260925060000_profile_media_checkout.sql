...
begin
  ...
  select * into v_media from public.profile_media
  where id=p_media_id and access_type='paid' and moderation_status='approved' for update;
  if not found then raise exception 'MEDIA_NOT_AVAILABLE'; end if;

  select * into v_profile from public.advertiser_profiles
  ...
  select * into v_purchase from public.profile_media_purchases
  ...
  if found and v_purchase.order_id is not null then
    ...
    if found and v_order.status='awaiting_payment' then
      return jsonb_build_object('already_owned',false,'existing_order',true,'order_id',v_order.id,'order_number',v_order.order_number,'media_id',v_media.id,'amount',v_media.price,'currency',v_media.currency);
    end if;
  end if;
  ...
  values(v_order_number,v_user,v_profile.id,v_media.price,0,0,v_media.price,v_media.currency,'awaiting_payment',
    jsonb_build_object('source','pecatho','kind','profile_media','product_type','profile_media','media_id',v_media.id,'profile_id',v_profile.id,'owner_user_id',v_profile.user_id,'title',coalesce(v_media.original_filename,'Conteúdo exclusivo'),'media_kind',v_media.kind))
  returning id into v_order_id;

  insert into public.payments(order_id,user_id,provider,amount,currency,status,payment_method,raw_reference)
  values(v_order_id,v_user,'pending',v_media.price,v_media.currency,'pending','pending','{}'::jsonb)
  ...
  return jsonb_build_object('already_owned',false,'existing_order',false,'order_id',v_order_id,'order_number',v_order_number,'media_id',v_media.id,'amount',v_media.price,'currency',v_media.currency);
end;
$$;
