drop function if exists public.search_public_advertisers(bigint,bigint,bigint,text,jsonb,text[],integer,integer,numeric,numeric,integer,integer,integer);
...
as $function$
  ...
  where ap.status::text = 'published'
    ...
    and not exists (
      select 1
      from jsonb_each(case when jsonb_typeof(p_attribute_filters)='object' then p_attribute_filters else '{}'::jsonb end) f(slug, wanted)
      where not exists (
        select 1
        from public.profile_attribute_values pav
        join public.category_attributes ca on ca.id = pav.attribute_id
        where pav.profile_id = ap.id
          and ca.category_id = ap.category_id
          and ca.slug = f.slug
          and ca.display_public = true
          and (
            pav.value = f.wanted
            or (jsonb_typeof(f.wanted)='array' and jsonb_typeof(pav.value)='array' and pav.value @> f.wanted)
...
revoke all on function public.search_public_advertisers(bigint,bigint,bigint,text,jsonb,text[],integer,integer,numeric,numeric,integer,integer,integer) from public;
grant execute on function public.search_public_advertisers(bigint,bigint,bigint,text,jsonb,text[],integer,integer,numeric,numeric,integer,integer,integer) to anon, authenticated;
