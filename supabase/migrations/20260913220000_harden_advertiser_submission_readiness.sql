...
create or replace function private.submit_advertiser_for_review(p_profile_id uuid)
...
begin
  ...
  if result.status not in ('draft','paused') then
    raise exception 'O anúncio não está em um estado que permita novo envio para análise';
  end if;

  update public.advertiser_profiles
  set status = 'pending_review'::profile_status,
      updated_at = now()
  where id = result.id
...
revoke all on function private.submit_advertiser_for_review(uuid) from public, anon, authenticated;
grant execute on function private.submit_advertiser_for_review(uuid) to postgres;
