-- Extend an existing Fans subscription only after an explicitly marked renewal is paid.
-- The renewal checkout creates a new paid purchase period while preserving the
-- current entitlement until provider confirmation.
...
begin
 ...
 v_product_id:=coalesce(nullif(v_meta->>'product_id','')::uuid,case when v_product_type='post' then nullif(v_meta->>'post_id','')::uuid when v_product_type='subscription' then nullif(v_meta->>'plan_id','')::uuid else null end);
 v_creator_id:=nullif(v_meta->>'creator_id','')::uuid; v_amount:=v_order.total;
 ...
 if p_payment_status='paid' then
   ...
   if v_product_type='post' then
     ...
     if v_subscription_id is null then
       insert into public.fans_subscriptions(creator_id,subscriber_user_id,plan_id,status,starts_at,ends_at,auto_renew) select v_creator_id,v_order.user_id,v_product_id,'active',pg_catalog.now(),pg_catalog.now()+(duration_days*interval '1 day'),false from public.fans_plans where id=v_product_id and status='active' returning id into v_subscription_id;
     end if;
     if v_subscription_id is null then raise exception 'PLAN_NOT_AVAILABLE'; end if;

     if coalesce((v_meta->>'renewal')::boolean, false) then
       update public.fans_subscriptions s
          set starts_at = case
            when s.ends_at is not null and s.ends_at > pg_catalog.now() then s.ends_at
            else pg_catalog.now()
          end,
          ends_at = (
            case
          ...
          ) + (pl.duration_days * interval '1 day'),
          status = 'active',
          updated_at = pg_catalog.now()
         from public.fans_plans pl
        where s.id = v_subscription_id
          and pl.id = v_product_id
 ...
 if p_payment_status in ('refunded','chargeback') then
   ...
   if found then
     ...
     if v_previous_purchase.subscription_id is not null then update public.fans_subscriptions set status='refunded',updated_at=pg_catalog.now() where id=v_previous_purchase.subscription_id; end if;
     if not exists(select 1 from public.fans_financial_ledger where purchase_id=v_previous_purchase.id and entry_type='refund_creator_credit' and status='posted') then
       insert into public.fans_financial_ledger(order_id,payment_id,purchase_id,subscription_id,creator_id,entry_type,direction,amount,currency,status,provider,provider_reference,metadata) values
