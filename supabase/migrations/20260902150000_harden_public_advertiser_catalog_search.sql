...
as $$
  ...
  where ap.status::text = 'published'
    ...
    and (p_price_min is null or exists (
      ...
      where jsonb_typeof(p->'price') in ('number','string') and (p->>'price') ~ '^([0-9]+([.][0-9]+)?|[.][0-9]+)$' and (p->>'price')::numeric >= p_price_min
    ))
    and (p_price_max is null or exists (
      select 1 from jsonb_array_elements(case when jsonb_typeof(ap.pricing->'periods')='array' then ap.pricing->'periods' else '[]'::jsonb end) p
      where jsonb_typeof(p->'price') in ('number','string') and (p->>'price') ~ '^([0-9]+([.][0-9]+)?|[.][0-9]+)$' and (p->>'price')::numeric <= p_price_max
    ))
    and not exists (
